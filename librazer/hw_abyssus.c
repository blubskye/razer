/*
 * Lowlevel hardware access for the Razer Abyssus mouse family.
 *
 * Important notice:
...
#define ABYSSUS_LOGO_NAME	"Logo LED"

#define ABYSSUS_MAX_RES		RAZER_MOUSE_RES_7200DPI
#define ABYSSUS_RES_STEP	RAZER_MOUSE_RES_100DPI
#define ABYSSUS_PACKET_MS	35
#define ABYSSUS_LED_NUM		1

static enum razer_mouse_freq abyssus_freqs[] = {
	RAZER_MOUSE_FREQ_125HZ, RAZER_MOUSE_FREQ_500HZ, RAZER_MOUSE_FREQ_1000HZ,
};

static enum razer_mouse_res abyssus_res_stages[] = {
	RAZER_MOUSE_RES_400DPI,
	RAZER_MOUSE_RES_800DPI,
	RAZER_MOUSE_RES_1800DPI,
	RAZER_MOUSE_RES_3500DPI,
	RAZER_MOUSE_RES_7200DPI,
};

#define ABYSSUS_DPIMAPPINGS_NUM	ARRAY_SIZE(abyssus_res_stages)
#define ABYSSUS_FREQS_NUM	ARRAY_SIZE(abyssus_freqs)
...
static struct abyssus_led *abyssus_get_led(struct abyssus_drv_data *d,
					   uint8_t id)
{
	if (id == d->logo_led.id)
		return &d->logo_led;
...
static int abyssus_send_init(struct razer_mouse *m)
{
	struct abyssus_drv_data *d = m->drv_data;
...
static int abyssus_send_set_resolution(struct razer_mouse *m)
{
	struct abyssus_drv_data *d = m->drv_data;
...
static int abyssus_send_set_frequency(struct razer_mouse *m)
{
	struct abyssus_drv_data *d = m->drv_data;
	int tfreq;
...
static int abyssus_send_set_led(struct razer_mouse *m,
				struct abyssus_led *led)
{
	struct abyssus_drv_data *d = m->drv_data;
...
static int abyssus_get_fw_version(struct razer_mouse *m)
{
	return -ENOSYS;
}

static struct razer_mouse_profile *abyssus_get_profiles(struct razer_mouse *m)
{
	return &((struct abyssus_drv_data *)m->drv_data)->profile;
}

static int abyssus_supported_axes(struct razer_mouse *m,
...
static int abyssus_supported_dpimappings(struct razer_mouse *m,
...
static int abyssus_supported_resolutions(struct razer_mouse *m,
					 enum razer_mouse_res **res_ptr)
{
	size_t i;
	size_t n = ABYSSUS_MAX_RES / ABYSSUS_RES_STEP;

	*res_ptr = calloc(n, sizeof(enum razer_mouse_res));
	if (!*res_ptr)
		return -ENOMEM;
	for (i = 0; i < n; i++)
		(*res_ptr)[i] = (i + 1) * ABYSSUS_RES_STEP;
	return n;
}

static int abyssus_supported_freqs(struct razer_mouse *m,
				   enum razer_mouse_freq **res_ptr)
{
	*res_ptr = malloc(sizeof(abyssus_freqs));
	if (!*res_ptr)
		return -ENOMEM;
	memcpy(*res_ptr, abyssus_freqs, sizeof(abyssus_freqs));
	return ABYSSUS_FREQS_NUM;
}

static enum razer_mouse_freq abyssus_get_freq(struct razer_mouse_profile *p)
{
	return ((struct abyssus_drv_data *)p->mouse->drv_data)->current_freq;
}

static struct razer_mouse_dpimapping *
abyssus_get_dpimapping(struct razer_mouse_profile *p, struct razer_axis *ax)
{
	return ((struct abyssus_drv_data *)p->mouse->drv_data)->current_dpimapping;
}

static int abyssus_change_dpimapping(struct razer_mouse_dpimapping *d,
				     enum razer_dimension dim,
				     enum razer_mouse_res res)
{
	struct abyssus_drv_data *priv = d->mouse->drv_data;

	if (!(d->dimension_mask & (1 << dim)))
		return -EINVAL;
	if (res == RAZER_MOUSE_RES_UNKNOWN)
		res = RAZER_MOUSE_RES_800DPI;
	if (res < ABYSSUS_RES_STEP || res > ABYSSUS_MAX_RES)
		return -EINVAL;
	d->res[dim] = res;
	if (d == priv->current_dpimapping)
		return abyssus_send_set_resolution(d->mouse);
	return 0;
}

static int abyssus_set_freq(struct razer_mouse_profile *p,
			    enum razer_mouse_freq freq)
{
	struct abyssus_drv_data *d = p->mouse->drv_data;
	...
	if (freq != RAZER_MOUSE_FREQ_125HZ &&
		...
		return -EINVAL;
	d->current_freq = freq;
	return abyssus_send_set_frequency(p->mouse);
}

static int abyssus_set_dpimapping(struct razer_mouse_profile *p,
				  ...
				  struct razer_mouse_dpimapping *dm)
{
	...
	return abyssus_send_set_resolution(p->mouse);
}

static int abyssus_led_toggle_state(struct razer_led *led,
				    enum razer_led_state new_state)
{
	int err;
	struct abyssus_drv_data *d = led->u.mouse->drv_data;
	struct abyssus_led *priv_led = abyssus_get_led(d, led->id);
...
static int abyssus_led_change_color(struct razer_led *led,
...
static int abyssus_led_set_mode(struct razer_led *led,
...
static int abyssus_get_leds(struct razer_mouse *m,
			    struct razer_led **leds_list)
{
	...
	*logo = (struct razer_led){
		...
		.u.mouse      = m,
		.next         = NULL,
		.toggle_state = abyssus_led_toggle_state,
		.change_color = abyssus_led_change_color,
		.set_mode     = abyssus_led_set_mode,
	};

	*leds_list = logo;
	return ABYSSUS_LED_NUM;
}
		       ...
		       struct libusb_device *usbdev)
{
	...
	if (libusb_get_device_descriptor(usbdev, &desc) == 0) {
		...
		case 0x0062: devname = "Abyssus 2000";              break;
		case 0x0058: devname = "Abyssus V2";                break;
		case 0x0097: devname = "Abyssus Elite DVA Edition"; break;
		case 0x009B: devname = "Abyssus Essential";         break;
		default:     devname = "Abyssus";                   break;
		}
	} else {
		devname = "Abyssus";
	}
	...
	razer_event_spacing_init(&d->packet_spacing, ABYSSUS_PACKET_MS);

	for (i = 0; i < ABYSSUS_DPIMAPPINGS_NUM; i++) {
		d->dpimappings[i] = (struct razer_mouse_dpimapping){
			.nr		= i,
			.change		= abyssus_change_dpimapping,
			.dimension_mask	= (1 << RAZER_DIM_X) | (1 << RAZER_DIM_Y),
			.mouse		= m,
		};
		d->dpimappings[i].res[RAZER_DIM_X] =
		d->dpimappings[i].res[RAZER_DIM_Y] = abyssus_res_stages[i];
	}

	d->current_dpimapping = &d->dpimappings[1]; /* default: 800 DPI */
	d->current_freq = RAZER_MOUSE_FREQ_500HZ;

	d->logo_led = (struct abyssus_led){
	...
	if ((err = abyssus_send_init(m)) ||
	    (err = abyssus_send_set_resolution(m)) ||
	    (err = abyssus_send_set_frequency(m)) ||
	    (err = abyssus_send_set_led(m, &d->logo_led)) ||
	...
	d->profile = (struct razer_mouse_profile){
		.mouse          = m,
		.get_freq       = abyssus_get_freq,
		.set_freq       = abyssus_set_freq,
		.get_dpimapping = abyssus_get_dpimapping,
		.set_dpimapping = abyssus_set_dpimapping,
	};
	...
	m->get_fw_version	= abyssus_get_fw_version;
	m->global_get_leds	= abyssus_get_leds;
	m->get_profiles		= abyssus_get_profiles;
	m->supported_axes	= abyssus_supported_axes;
	m->supported_resolutions = abyssus_supported_resolutions;
	m->supported_freqs	= abyssus_supported_freqs;
	m->supported_dpimappings = abyssus_supported_dpimappings;
