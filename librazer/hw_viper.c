/*
 * Lowlevel hardware access for the Razer Viper mouse family.
 *
 * Important notice:
...
#define VIPER_SCROLL_NAME	"Scrollwheel"
#define VIPER_LOGO_NAME		"GlowingLogo"

#define VIPER_MAX_RES		RAZER_MOUSE_RES_20000DPI
#define VIPER_RES_STEP		RAZER_MOUSE_RES_100DPI
#define VIPER_PACKET_MS		35
#define VIPER_LED_NUM		2

static enum razer_mouse_freq viper_freqs[] = {
	RAZER_MOUSE_FREQ_125HZ, RAZER_MOUSE_FREQ_500HZ, RAZER_MOUSE_FREQ_1000HZ,
};

static enum razer_mouse_res viper_res_stages[] = {
	RAZER_MOUSE_RES_400DPI,   RAZER_MOUSE_RES_800DPI,
	RAZER_MOUSE_RES_1600DPI,  RAZER_MOUSE_RES_3200DPI,
	RAZER_MOUSE_RES_8500DPI,  RAZER_MOUSE_RES_20000DPI,
};

#define VIPER_DPIMAPPINGS_NUM	ARRAY_SIZE(viper_res_stages)
#define VIPER_FREQS_NUM		ARRAY_SIZE(viper_freqs)
...
static struct viper_led *viper_get_led(struct viper_drv_data *d,
				       uint8_t id)
{
...
static int viper_send_init(struct razer_mouse *m)
{
	struct viper_drv_data *d = m->drv_data;
...
static int viper_send_set_resolution(struct razer_mouse *m)
{
	struct viper_drv_data *d = m->drv_data;
...
static int viper_send_set_frequency(struct razer_mouse *m)
{
	struct viper_drv_data *d = m->drv_data;
	int tfreq;
...
static int viper_send_set_led(struct razer_mouse *m,
			      struct viper_led *led)
{
	struct viper_drv_data *d = m->drv_data;
...
static int viper_get_fw_version(struct razer_mouse *m)
{
	return -ENOSYS;
}

static struct razer_mouse_profile *viper_get_profiles(struct razer_mouse *m)
{
	return &((struct viper_drv_data *)m->drv_data)->profile;
}

static int viper_supported_axes(struct razer_mouse *m,
...
static int viper_supported_dpimappings(struct razer_mouse *m,
...
static int viper_supported_resolutions(struct razer_mouse *m,
				       enum razer_mouse_res **res_ptr)
{
	size_t i;
	size_t n = VIPER_MAX_RES / VIPER_RES_STEP;

	*res_ptr = calloc(n, sizeof(enum razer_mouse_res));
	if (!*res_ptr)
		return -ENOMEM;
	for (i = 0; i < n; i++)
		(*res_ptr)[i] = (i + 1) * VIPER_RES_STEP;
	return n;
}

static int viper_supported_freqs(struct razer_mouse *m,
				 enum razer_mouse_freq **res_ptr)
{
	*res_ptr = malloc(sizeof(viper_freqs));
	if (!*res_ptr)
		return -ENOMEM;
	memcpy(*res_ptr, viper_freqs, sizeof(viper_freqs));
	return VIPER_FREQS_NUM;
}

static enum razer_mouse_freq viper_get_freq(struct razer_mouse_profile *p)
{
	return ((struct viper_drv_data *)p->mouse->drv_data)->current_freq;
}

static struct razer_mouse_dpimapping *
viper_get_dpimapping(struct razer_mouse_profile *p, struct razer_axis *ax)
{
	return ((struct viper_drv_data *)p->mouse->drv_data)->current_dpimapping;
}

static int viper_change_dpimapping(struct razer_mouse_dpimapping *d,
				   enum razer_dimension dim,
				   enum razer_mouse_res res)
{
	struct viper_drv_data *priv = d->mouse->drv_data;

	if (!(d->dimension_mask & (1 << dim)))
		return -EINVAL;
	if (res == RAZER_MOUSE_RES_UNKNOWN)
		res = RAZER_MOUSE_RES_1600DPI;
	if (res < VIPER_RES_STEP || res > VIPER_MAX_RES)
		return -EINVAL;
	d->res[dim] = res;
	if (d == priv->current_dpimapping)
		return viper_send_set_resolution(d->mouse);
	return 0;
}

static int viper_set_freq(struct razer_mouse_profile *p,
			  enum razer_mouse_freq freq)
{
	struct viper_drv_data *d = p->mouse->drv_data;
	...
	if (freq != RAZER_MOUSE_FREQ_125HZ &&
		...
		return -EINVAL;
	d->current_freq = freq;
	return viper_send_set_frequency(p->mouse);
}

static int viper_set_dpimapping(struct razer_mouse_profile *p,
				...
				struct razer_mouse_dpimapping *dm)
{
	...
	return viper_send_set_resolution(p->mouse);
}

static int viper_led_toggle_state(struct razer_led *led,
				  enum razer_led_state new_state)
{
	int err;
	struct viper_drv_data *d = led->u.mouse->drv_data;
	struct viper_led *priv_led = viper_get_led(d, led->id);
...
static int viper_led_change_color(struct razer_led *led,
...
static int viper_led_set_mode(struct razer_led *led,
...
static int viper_get_leds(struct razer_mouse *m,
			  struct razer_led **leds_list)
{
	...
	*scroll = (struct razer_led){
		...
		.u.mouse      = m,
		.next         = logo,
		.toggle_state = viper_led_toggle_state,
		.change_color = viper_led_change_color,
		.set_mode     = viper_led_set_mode,
	};

	*logo = (struct razer_led){
		.id	= d->logo_led.id,
		.name	= VIPER_LOGO_NAME,
		     ...
		     struct libusb_device *usbdev)
{
	...
	if (libusb_get_device_descriptor(usbdev, &desc) == 0) {
		...
		case 0x0078: devname = "Viper";                       break;
		case 0x008A: devname = "Viper Mini";                  break;
		case 0x0073: devname = "Viper Ultimate (wired)";      break;
		case 0x0074: devname = "Viper Ultimate (wireless)";   break;
		case 0x009E: devname = "Viper 8KHz";                  break;
		default:     devname = "Viper";                       break;
		}
	} else {
		devname = "Viper";
	}
	...
	if ((err = viper_send_init(m)) ||
	    (err = viper_send_set_resolution(m)) ||
	    (err = viper_send_set_frequency(m)) ||
	    (err = viper_send_set_led(m, &d->scroll_led)) ||
	    (err = razer_chroma_ext_set_static_color(m, &d->packet_spacing,
						     d->scroll_led.id,
						     d->scroll_led.color.r,
						     d->scroll_led.color.g,
						     d->scroll_led.color.b)) ||
	    (err = viper_send_set_led(m, &d->logo_led)) ||
	    (err = razer_chroma_ext_set_static_color(m, &d->packet_spacing,
						     d->logo_led.id,
						     d->logo_led.color.r,
						     d->logo_led.color.g,
	...
	d->profile = (struct razer_mouse_profile){
		.mouse          = m,
		.get_freq       = viper_get_freq,
		.set_freq       = viper_set_freq,
		.get_dpimapping = viper_get_dpimapping,
		.set_dpimapping = viper_set_dpimapping,
	};
	...
	m->get_fw_version	= viper_get_fw_version;
	m->global_get_leds	= viper_get_leds;
	m->get_profiles		= viper_get_profiles;
	m->supported_axes	= viper_supported_axes;
	m->supported_resolutions = viper_supported_resolutions;
	m->supported_freqs	= viper_supported_freqs;
	m->supported_dpimappings = viper_supported_dpimappings;
