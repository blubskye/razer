/*
 * Lowlevel hardware access for the Razer Basilisk mouse family.
 *
 * Important notice:
...
#define BASILISK_SCROLL_NAME	"Scrollwheel"
#define BASILISK_LOGO_NAME	"GlowingLogo"

#define BASILISK_MAX_RES	RAZER_MOUSE_RES_20000DPI
#define BASILISK_RES_STEP	RAZER_MOUSE_RES_100DPI
#define BASILISK_PACKET_MS	35
#define BASILISK_LED_NUM	2

static enum razer_mouse_freq basilisk_freqs[] = {
	RAZER_MOUSE_FREQ_125HZ, RAZER_MOUSE_FREQ_500HZ, RAZER_MOUSE_FREQ_1000HZ,
};

static enum razer_mouse_res basilisk_res_stages[] = {
...
#define BASILISK_DPIMAPPINGS_NUM	ARRAY_SIZE(basilisk_res_stages)
#define BASILISK_FREQS_NUM		ARRAY_SIZE(basilisk_freqs)
...
static struct basilisk_led *basilisk_get_led(struct basilisk_drv_data *d,
					     uint8_t id)
{
...
static int basilisk_send_init(struct razer_mouse *m)
{
	struct basilisk_drv_data *d = m->drv_data;
...
static int basilisk_send_set_resolution(struct razer_mouse *m)
{
	struct basilisk_drv_data *d = m->drv_data;
...
static int basilisk_send_set_frequency(struct razer_mouse *m)
{
	struct basilisk_drv_data *d = m->drv_data;
	int tfreq;
...
static int basilisk_send_set_led(struct razer_mouse *m,
				 struct basilisk_led *led)
{
	struct basilisk_drv_data *d = m->drv_data;
...
static int basilisk_get_fw_version(struct razer_mouse *m)
{
	return -ENOSYS;
}

static struct razer_mouse_profile *basilisk_get_profiles(struct razer_mouse *m)
{
	return &((struct basilisk_drv_data *)m->drv_data)->profile;
}

static int basilisk_supported_axes(struct razer_mouse *m,
...
static int basilisk_supported_dpimappings(struct razer_mouse *m,
...
static int basilisk_supported_resolutions(struct razer_mouse *m,
					  enum razer_mouse_res **res_ptr)
{
	size_t i;
	size_t n = BASILISK_MAX_RES / BASILISK_RES_STEP;

	*res_ptr = calloc(n, sizeof(enum razer_mouse_res));
	if (!*res_ptr)
		return -ENOMEM;
	for (i = 0; i < n; i++)
		(*res_ptr)[i] = (i + 1) * BASILISK_RES_STEP;
	return n;
}

static int basilisk_supported_freqs(struct razer_mouse *m,
				    enum razer_mouse_freq **res_ptr)
{
	*res_ptr = malloc(sizeof(basilisk_freqs));
	if (!*res_ptr)
		return -ENOMEM;
	memcpy(*res_ptr, basilisk_freqs, sizeof(basilisk_freqs));
	return BASILISK_FREQS_NUM;
}

static enum razer_mouse_freq basilisk_get_freq(struct razer_mouse_profile *p)
{
	return ((struct basilisk_drv_data *)p->mouse->drv_data)->current_freq;
}

static struct razer_mouse_dpimapping *
basilisk_get_dpimapping(struct razer_mouse_profile *p, struct razer_axis *ax)
{
	return ((struct basilisk_drv_data *)p->mouse->drv_data)->current_dpimapping;
}

static int basilisk_change_dpimapping(struct razer_mouse_dpimapping *d,
				      enum razer_dimension dim,
				      enum razer_mouse_res res)
{
	struct basilisk_drv_data *priv = d->mouse->drv_data;

	if (!(d->dimension_mask & (1 << dim)))
		return -EINVAL;
	if (res == RAZER_MOUSE_RES_UNKNOWN)
		res = RAZER_MOUSE_RES_1600DPI;
	if (res < BASILISK_RES_STEP || res > BASILISK_MAX_RES)
		return -EINVAL;
	d->res[dim] = res;
	if (d == priv->current_dpimapping)
		return basilisk_send_set_resolution(d->mouse);
	return 0;
}

static int basilisk_set_freq(struct razer_mouse_profile *p,
			     enum razer_mouse_freq freq)
{
	struct basilisk_drv_data *d = p->mouse->drv_data;
	...
	if (freq != RAZER_MOUSE_FREQ_125HZ &&
		...
		return -EINVAL;
	d->current_freq = freq;
	return basilisk_send_set_frequency(p->mouse);
}

static int basilisk_set_dpimapping(struct razer_mouse_profile *p,
				   ...
				   struct razer_mouse_dpimapping *dm)
{
	...
	return basilisk_send_set_resolution(p->mouse);
}

static int basilisk_led_toggle_state(struct razer_led *led,
				     enum razer_led_state new_state)
{
	int err;
	struct basilisk_drv_data *d = led->u.mouse->drv_data;
	struct basilisk_led *priv_led = basilisk_get_led(d, led->id);
...
static int basilisk_led_change_color(struct razer_led *led,
...
static int basilisk_led_set_mode(struct razer_led *led,
...
static int basilisk_get_leds(struct razer_mouse *m,
			     struct razer_led **leds_list)
{
	...
	*scroll = (struct razer_led){
		...
		.u.mouse      = m,
		.next         = logo,
		.toggle_state = basilisk_led_toggle_state,
		.change_color = basilisk_led_change_color,
		.set_mode     = basilisk_led_set_mode,
	};

	*logo = (struct razer_led){
		.id	= d->logo_led.id,
		.name	= BASILISK_LOGO_NAME,
			...
			struct libusb_device *usbdev)
{
	...
	if (libusb_get_device_descriptor(usbdev, &desc) == 0) {
		...
		case 0x0064: devname = "Basilisk";                    break;
		case 0x0086: devname = "Basilisk Essential";          break;
		case 0x0088: devname = "Basilisk X HyperSpeed";       break;
		case 0x0099: devname = "Basilisk V2";                 break;
		case 0x0083: devname = "Basilisk V3";                 break;
		case 0x00AB: devname = "Basilisk V3 Pro (wired)";     break;
		case 0x00B7: devname = "Basilisk V3 HyperSpeed";      break;
		case 0x00AA: devname = "Basilisk V3 X HyperSpeed";    break;
		default:     devname = "Basilisk";                    break;
		}
	} else {
		devname = "Basilisk";
	}
	...
	if ((err = basilisk_send_init(m)) ||
	    (err = basilisk_send_set_resolution(m)) ||
	    (err = basilisk_send_set_frequency(m)) ||
	    (err = basilisk_send_set_led(m, &d->scroll_led)) ||
	    (err = razer_chroma_ext_set_static_color(m, &d->packet_spacing,
						     d->scroll_led.id,
						     d->scroll_led.color.r,
						     d->scroll_led.color.g,
						     d->scroll_led.color.b)) ||
	    (err = basilisk_send_set_led(m, &d->logo_led)) ||
	    (err = razer_chroma_ext_set_static_color(m, &d->packet_spacing,
						     d->logo_led.id,
						     d->logo_led.color.r,
						     d->logo_led.color.g,
	...
	d->profile = (struct razer_mouse_profile){
		.mouse          = m,
		.get_freq       = basilisk_get_freq,
		.set_freq       = basilisk_set_freq,
		.get_dpimapping = basilisk_get_dpimapping,
		.set_dpimapping = basilisk_set_dpimapping,
	};
	...
	m->get_fw_version	= basilisk_get_fw_version;
	m->global_get_leds	= basilisk_get_leds;
	m->get_profiles		= basilisk_get_profiles;
	m->supported_axes	= basilisk_supported_axes;
	m->supported_resolutions = basilisk_supported_resolutions;
	m->supported_freqs	= basilisk_supported_freqs;
	m->supported_dpimappings = basilisk_supported_dpimappings;
